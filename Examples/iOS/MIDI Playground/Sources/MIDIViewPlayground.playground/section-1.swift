...
class PianoView: UIView {
	...
	func drawBlack(noteNumber: Int, offset: CGFloat) {
		UIColor.black.setFill()
		UIBezierPath(rect: CGRect(x: offset-blackKeyWidth/2.0, y: 0, width: blackKeyWidth, height: self.bounds.height * 0.6)).fill()
	}
	
	func drawWhite(noteNumber: Int, offset: CGFloat) {
		UIColor.gray.setFill()
		UIBezierPath(rect: CGRect(x: offset+self.whiteKeyWidth-1, y: 0, width: 2, height: self.bounds.height)).fill()
	}
	...
	override func draw(_ rect: CGRect) {
		...
		for noteNumber in 1..<numKeys {
			if noteIsBlack(noteNumber) {
				drawBlack(noteNumber: noteNumber, offset: offset)
			} else {
	...
	var whiteKeyWidth: CGFloat {
		...
		let numWhiteKeys = (1...numberOfKeys).filter { !noteIsBlack($0) }.count
		return width / CGFloat(numWhiteKeys)
	}
	
	var blackKeyWidth: CGFloat {
		return whiteKeyWidth * 0.583
	}
}
